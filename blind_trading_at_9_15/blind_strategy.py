from common import write_log, get_object_from_list, get_percentage_value, get_formatted_price
from util import get_stocks_info, get_available_balance, create_order_single


def start_strategy(props, fyers, symbols):
    write_log("\n")
    strategy_obj = props["first_candle_same_trend_strategy"]
    stocks_details = get_stocks_info(props, fyers, symbols)
    for symbolObj in stocks_details:
        ...
        create_order_single(props, fyers, symbolObj["n"], qty, order_type, side, limit_price, 0, stop_loss, 0)
